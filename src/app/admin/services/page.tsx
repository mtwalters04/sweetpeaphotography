import Link from 'next/link';
import { createClient } from '@/lib/supabase/server';
import { formatUsd } from '@/lib/money';

export const metadata = { title: 'Services · Admin' };

export default async function ServicesIndex() {
  const supabase = await createClient();
  const { data: services } = await supabase
    .from('session_types')
    .select('id, slug, name, starting_at_cents, duration_minutes, active, order_index')
    .order('order_index')
    .order('name');

  return (
    <>
      <div className="flex items-center justify-between mb-10">
        <div>
          <p className="text-ash text-t-12 eyebrow-label mb-2">Catalog</p>
          <h2 className="font-serif text-t-28">Services</h2>
        </div>
        <Link
          href="/admin/services/new"
          className="text-t-12 eyebrow-label text-ink border-b border-ink pb-1 hover:text-accent hover:border-accent"
        >
          New →
        </Link>
      </div>

      {services && services.length > 0 ? (
        <ul className="border-y border-mist divide-y divide-mist">
          {services.map((s) => (
            <li key={s.id}>
              <Link
                href={`/admin/services/${s.id}/edit`}
                className="grid grid-cols-12 gap-6 py-5 items-baseline hover:text-accent transition-colors"
              >
                <div className="col-span-12 md:col-span-7">
                  <p className="font-serif text-t-22">{s.name}</p>
                  <p className="text-ash text-t-12 eyebrow-label mt-1">/{s.slug}</p>
                </div>
                <p className="col-span-6 md:col-span-3 text-ash text-t-14 font-light">
                  {s.duration_minutes} min
                </p>
                <p className="col-span-6 md:col-span-2 text-right md:text-left font-serif text-t-22">
                  {formatUsd(s.starting_at_cents)}
                </p>
              </Link>
            </li>
          ))}
        </ul>
      ) : (
        <div className="border-y border-mist py-16 text-center">
          <p className="text-t-22 text-ash">No services yet.</p>
          <Link
            href="/admin/services/new"
            className="inline-block mt-6 text-t-14 underline hover:text-accent"
          >
            Add the first one →
          </Link>
        </div>
      )}
    </>
  );
}
